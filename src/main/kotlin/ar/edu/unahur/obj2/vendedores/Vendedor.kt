...
class Viajante(val provinciasHabilitadas: List<Provincia>) : Vendedor() {
  ...
  override fun esInfluyente() = this.poblacionDeProvinciasHabilitadas() >= 10000000

  fun poblacionDeProvinciasHabilitadas() = provinciasHabilitadas.sumBy { p -> p.poblacion }
}
...
class ComercioCorresponsal(val ciudades: List<Ciudad>) : Vendedor() {
  ...
  override fun esInfluyente() = this.ciudadesConSucursales() >= 5 || this.provinciasConSucursales() >= 3

  fun ciudadesConSucursales() = ciudades.size

  fun provinciasConSucursales() = ciudades.map{ c -> c.provincia}.toSet().size
}
